...
1. 在游戏选歌界面，把目标歌曲滚动到屏幕中央，**用截图工具截下歌曲卡片或歌曲名标题那一小块**（保留文字 + 一点点背景即可，不要带 UID 或个人信息）。
2. 把图片保存为 `<歌曲名>.png` 放到 `assets/song_templates/`，文件名就是 GUI 下拉框里显示的名字。
3. 重启工具，下拉框就会出现这首歌。

仓库自带几张参考模板（`Destiny.png` / `Heroic_Appearance.png` 等）；不同分辨率/语言下可能匹配率偏低，建议自己重新截一张。

### 在 GUI 里启用

- 勾选「**启用自动选歌**」并选歌曲。
- 想刷多次就再勾「**启用自动连打**」并填次数；连打靠结算界面后发 ESC 回选歌再来一次，需要同时开自动选歌。
...
进入选歌界面后才会触发；如果一直停在 idle，多半是模板匹配不到——重新截一张更清楚的、或调低阈值。
